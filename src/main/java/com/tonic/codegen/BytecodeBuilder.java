...
import com.tonic.utill.InsnUtil;
...
public class BytecodeBuilder {
    ...
    public BytecodeBuilder jumpIfICmp(ConditionType conditionType, LabelNode labelNode)
    {
        insnList.add(new JumpInsnNode(conditionType.getIcmpOpcode(), labelNode));
        return this;
    }
    ...
    public BytecodeBuilder pushFrame(int type, Object[] locals, Object[] stack)
    {
        int numLocals = locals == null ? 0 : locals.length;
        int numStack = stack == null ? 0 : stack.length;
        insnList.add(new FrameNode(type, numLocals, locals, numStack, stack));
        return this;
    }

    /**
     * A full frame (used in Java 7+).
     * @param locals locals dif
     * @param stack stack dif
     * @return builder
     */
    public BytecodeBuilder f_new(Object[] locals, Object[] stack)
    {
        return pushFrame(F_NEW, locals, stack);
    }

    /**
     * A full frame with explicit local variables and stack.
     * @param locals locals dif
     * @param stack stack dif
     * @return builder
     */
    public BytecodeBuilder f_full(Object[] locals, Object[] stack)
    {
        return pushFrame(F_FULL, locals, stack);
    }

    /**
     * A frame that appends new local variables to the previous frame.
     * @param locals locals dif
     * @param stack stack dif
     * @return builder
     */
    public BytecodeBuilder f_append(Object[] locals, Object[] stack)
    {
        return pushFrame(F_APPEND, locals, stack);
    }

    /**
     * The frame is the same as the previous one.
     * @return builder
     */
    public BytecodeBuilder f_same()
    {
        return pushFrame(F_SAME, null, null);
    }

    /**
     * The frame is the same as the previous one, except for one stack element.
     * @param stack stack dif
     * @return builder
     */
    public BytecodeBuilder f_same1(Object[] stack)
    {
        return pushFrame(F_SAME1, null, stack);
    }

    /**
     * Removes one or more local variables from the end of the local variables array.
     * @param numLocals locals dif (to remove)
     * @return builder
     */
    public BytecodeBuilder f_chop(int numLocals)
    {
        insnList.add(new FrameNode(F_CHOP, numLocals, null, 0, null));
        return this;
    }

    public BytecodeBuilder multiANewArray(String type, int i)
    {
        insnList.add(new MultiANewArrayInsnNode(type, i));
        return this;
    }
}
